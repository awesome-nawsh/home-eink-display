...
- The web UI's "Restart Service" button (`/api/restart`) needs a one-time passwordless-sudo entry on the Pi: `sudo cp systemd/bus_display_restart.sudoers.example /etc/sudoers.d/bus_display_restart && sudo chmod 440 /etc/sudoers.d/bus_display_restart && sudo visudo -c`. `/api/restart` uses `systemctl restart --no-block` (returns as soon as the restart is queued, not once bus_display is fully back up — sudoers matches the command line exactly, so a deployment set up before this line was added needs to re-run the copy step to pick up `--no-block`).
