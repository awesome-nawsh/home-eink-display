...
**Decision**: fetchers never propagate exceptions up to the render layer. Every failure path returns *something renderable*: the last good cached value (served past its normal TTL for up to `STALE_DATA_MAX_AGE` during an API outage), or — once even stale data is too old to trust — `None` as an explicit "the API is failing" sentinel that the bus/train draw functions render as a visible "data unavailable" error section. `None` is deliberately distinct from a successful-but-empty response (no buses running is real data and renders blank, not as an error). Weather is the exception to the age limit: a weather reading ages gracefully, so last-known-good weather is served indefinitely during an HA outage.
...
## 7. Reliability model: systemd watchdog, not in-process self-healing

**Decision**: hang detection and recovery both belong to systemd. The unit runs with `Type=notify` + `WatchdogSec=900`; the main loop pings `sd_notify('WATCHDOG=1')` every tick, and if the pings stop systemd kills and restarts the process. The app does **not** attempt to reset internal state, restart threads, or recover in-process.

**History**: the original design used an in-process `Watchdog` class, but it was structurally inert — the loop fed it and checked it from the same thread, so elapsed time was always ~0, and a genuinely hung loop never reached the check at all. The v14 review pass replaced it with the systemd mechanism, which watches the process from outside and can actually kill a wedged one. `WatchdogSec` (900s) must comfortably exceed the longest legitimate gap between ticks: `SLEEP_INTERVAL` (default 300s) plus a full e-ink refresh.
