...
- **Air quality** (post-merge follow-up): NEA 24-hour PSI (data.gov.sg, free, no key — worst of the five regional readings) shown next to humidity on the `bus_train_screen` weather section and in the `daytime_screen` detail line. The PSI rides along inside the weather dict (fetched/cached with it); a PSI fetch failure just hides the reading, never breaks weather. A PSI change also triggers a daytime redraw.
