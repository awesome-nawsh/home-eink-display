...
| `daytime_screen` | `display_daytime_screen()`, `app/render/daytime_screen.py` | Per its own `schedule_config.json` window — beats `ha_screen` on overlap and is the safe fallback when no window matches at all; shown when `sleep_screen` isn't active and `bus_train_screen` isn't eligible/scheduled | Placeholder only: `"Day time screen"` centered (`anchor="mm"`) on the black buffer, Bold `FONT_HEADER` (28). Real content deferred (see `todo.md`). Static — only redrawn on screen-entry or manual refresh, not every loop tick. |
