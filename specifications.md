...
| Web config: screen preview | `/preview` page renders any of `combined`/`daytime`/`sleep`/`boot`/`debug` (not `ha_screen`, which needs a live HA fetch) to a PNG in the browser, with scenario toggles (train disruption, weather unavailable, no journey time, manual refresh) — the same hardware-free code path as `tools/preview_render.py`, so no SSH access or waiting for a screen's scheduled window is needed to see what it looks like | none — read-only, no `.env`/schedule changes |
